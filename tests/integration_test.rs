...
async fn test_root_entry_child_id_stays_in_sync() {
    ...
    let root_data =
        nanofile::storage::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_fs_id)
            .await
            .unwrap();
    ...
    let root_data_v2 =
        nanofile::storage::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_fs_id_v2)
            .await
            .unwrap();
...
async fn test_regression_rename_file_creates_new_commit() {
    ...
    let root_data =
        nanofile::storage::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_after)
            .await
            .unwrap();
