...
async fn test_regression_commit_root_contains_full_tree() {
    ...
    let entries = nanofile::serialization::pack_fs::decode_pack_fs_entries(&packed).unwrap();
    assert_eq!(entries.len(), 1);

    let decompressed = nanofile::serialization::pack_fs::decompress_fs_data(&entries[0].1).unwrap();
...
async fn test_regression_cdc_determinism() {
    ...
    let root_data: serde_json::Value = serde_json::from_slice(
        &nanofile::serialization::pack_fs::decompress_fs_data(&entries[0].1).unwrap(),
    )
    .unwrap();
    ...
    for d in root_data["dirents"].as_array().unwrap() {
        ...
        let packed = resp.bytes().await.unwrap();
        let child_entries =
            nanofile::serialization::pack_fs::decode_pack_fs_entries(&packed).unwrap();
        let child_data: serde_json::Value = serde_json::from_slice(
            &nanofile::serialization::pack_fs::decompress_fs_data(&child_entries[0].1).unwrap(),
        )
        .unwrap();
...
async fn test_regression_cdc_chunk_size_bounds() {
    ...
    let child_entries =
        nanofile::serialization::pack_fs::decode_pack_fs_entries(&child_packed).unwrap();
...
async fn test_root_entry_child_id_stays_in_sync() {
    ...
    let root_data = nanofile::repo::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_fs_id)
        .await
        .unwrap();
    ...
    let root_data_v2 =
        nanofile::repo::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_fs_id_v2)
            .await
            .unwrap();
...
async fn test_regression_rename_file_creates_new_commit() {
    ...
    let root_data = nanofile::repo::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_after)
        .await
        .unwrap();
...
async fn test_empty_directory_uses_emtpysha1() {
    ...
    let api_fs_obj = nanofile::entity::fs_object::Entity::find()
        .filter(
            nanofile::entity::fs_object::Column::FsId
