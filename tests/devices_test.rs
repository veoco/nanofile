...
async fn test_list_devices_with_data() {
    ...
    let s2fa = nanofile::entity::s2fa_token::ActiveModel {
        id: sea_orm::NotSet,
    ...
    let sync_tokens = nanofile::entity::sync_token::Entity::find()
        .filter(nanofile::entity::sync_token::Column::RepoId.eq(&repo_id))
    ...
    let mut sync_active: nanofile::entity::sync_token::ActiveModel = sync_tokens[0].clone().into();
    ...
    let count = nanofile::entity::sync_token::Entity::find()
        .filter(nanofile::entity::sync_token::Column::PeerId.eq("phone-001"))
        .count(server.db.as_ref())
        .await
        .unwrap();
