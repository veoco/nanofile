    ...
    match cli.command.unwrap_or(Command::Server) {
        Command::Server => {
            ...
            if let (Some(admin_email), Some(admin_password)) = (
                ...
                let count = nanofile_infra::entity::user::Entity::find()
                    .count(state.db.as_ref())
                    .await?;
                if count == 0 {
                    tracing::info!("No users found; creating initial admin user");
                    let password_hash = nanofile_server::auth::password::hash_password(
                        admin_password,
                        state.config.auth.password_hash_iterations,
                    );
                    let now = chrono::Utc::now().timestamp();
                    let model = nanofile_infra::entity::user::ActiveModel {
                        id: sea_orm::NotSet,
            ...
            let app = Router::new()
                ...
                .merge(nanofile_server::user::handler::avatar::image_routes())
                .route(
                    "/static/{*path}",
                    get(nanofile_server::static_assets::serve_static),
                )
        ...
        Command::Adduser {
            ...
            use nanofile_infra::entity::user;
