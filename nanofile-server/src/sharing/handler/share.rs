...
use crate::sharing::service::share_service;
...
) -> Result<Json<Vec<share_service::ShareLinkInfo>>, AppError> {
    let infos = share_service::list_share_links(&state.repos, auth.user_id).await?;
    Ok(Json(infos))
}
...
) -> Result<Json<share_service::ShareLinkInfo>, AppError> {
    let info = share_service::create_share_link(
        state.db.as_ref(),
        &state.repos,
        &state.config,
        &req.repo_id,
...
pub async fn delete_share_link(
    ...
    share_service::delete_share_link(state.db.as_ref(), &token, auth.user_id).await?;
    Ok(())
}
...
pub async fn beshare_repo(
    ...
    let result = share_service::beshare_repo(
        state.db.as_ref(),
        &state.repos,
...
pub async fn list_share_members(
    ...
    State(state): State<Arc<AppState>>,
    Path(repo_id): Path<String>,
) -> Result<Json<Vec<share_service::ShareMember>>, AppError> {
    ...
    let members = share_service::list_share_members(&state.repos, &repo_id).await?;
    Ok(Json(members))
}
...
pub async fn modify_share_permission(
    ...
    share_service::modify_share_permission(
        state.db.as_ref(),
        &state.repos,
        state.notification_manager.as_ref(),
        &repo_id,
