...
use crate::sharing::service::group_service;
...
pub async fn list_groups(
    ...
    let result = group_service::list_groups(state.db.as_ref(), &state.repos, auth.user_id).await?;
    Ok(Json(result))
}
...
pub async fn search_user(
    ...
    let result = group_service::search_user(state.db.as_ref(), &state.repos, &q).await?;
    Ok(Json(result))
}
