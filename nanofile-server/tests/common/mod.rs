...
fn cached_hash_password(password: &str) -> String {
    ...
    let h = nanofile_server::auth::password::hash_password(password, TEST_PBKDF2_ITERATIONS);
...
impl TestServer {
    ...
    async fn start_with_config(
        ...
        let config = nanofile_server::config::Config {
            server: nanofile_server::config::ServerConfig {
            ...
            database: nanofile_server::config::DatabaseConfig {
                url: "sqlite::memory:".to_string(),
            },
            storage: nanofile_server::config::StorageConfig {
            ...
            auth: nanofile_server::config::AuthConfig {
            ...
            logging: nanofile_server::config::LoggingConfig {
                level: "debug".to_string(),
            },
            gc: nanofile_server::config::GcConfig {
                enabled: false,
                interval_hours: 24,
                keep_commits: 10,
            },
            notification: nanofile_server::config::NotificationConfig {
            ...
            index: nanofile_server::config::IndexConfig {
        ...
        let sync_routes = nanofile_server::sync::sync_routes();
        let sdoc_routes = nanofile_server::sdoc::sdoc_routes();
        let web_routes = nanofile_server::web::web_routes();
        let ui_routes = nanofile_server::ui::ui_routes();
        let notification_routes = nanofile_server::notification::notification_routes();

        let app = Router::new()
            .merge(nanofile_server::routes::api_routes())
            .merge(sync_routes)
            .merge(sdoc_routes)
            .merge(web_routes)
            .merge(ui_routes)
            ...
            .route(
                "/static/{*path}",
                axum::routing::get(nanofile_server::static_assets::serve_static),
            )
