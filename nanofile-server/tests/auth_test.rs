...
async fn test_login_2fa_required_no_otp() {
    ...
    let user_2fa = nanofile_server::entity::user_2fa::ActiveModel {
...
fn generate_valid_totp() -> String {
    let totp =
        nanofile_server::auth::totp::TotpManager::create_totp(totp_secret(), "test@example.com", "")
            .unwrap();
...
async fn test_s2fa_expired_token() {
    ...
    let expired_model = nanofile_server::entity::s2fa_token::ActiveModel {
        id: sea_orm::NotSet,
        user_id: sea_orm::Set(1),
    ...
    let count = nanofile_server::entity::s2fa_token::Entity::find()
        .filter(nanofile_server::entity::s2fa_token::Column::Token.eq(expired_token))
