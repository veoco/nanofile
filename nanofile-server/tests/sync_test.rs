...
async fn test_head_commits_multi_with_updates() {
    ...
    let commit = nanofile_server::entity::commit::ActiveModel {
        id: sea_orm::NotSet,
    ...
    // Update repo HEAD
    let mut repo: nanofile_server::entity::repo::ActiveModel =
        nanofile_server::entity::repo::Entity::find_by_id(&repo_id)
...
async fn test_pack_fs_accepts_json_array() {
    ...
    let empty_dir = nanofile_server::serialization::fs_json::FsDirData {
        dirents: vec![],
        obj_type: 3,
        version: 1,
    };
    let dir_json = empty_dir.to_compact_json();
    let dir_fs_id = nanofile_server::crypto::fs_id::sha1_hex(dir_json.as_bytes());
    let dir_compressed =
        nanofile_server::serialization::pack_fs::compress_fs_data(dir_json.as_bytes()).unwrap();
    ...
    let decoded = nanofile_server::serialization::pack_fs::decode_pack_fs_entries(&data).unwrap();
...
async fn test_regression_recv_fs_accepts_dir_type_3() {
    ...
    use nanofile_server::serialization::fs_json::{FsDirData, FsFileData};
    ...
    let dir_data = FsDirData {
        dirents: vec![nanofile_server::serialization::fs_json::DirEntryData {
            id: file_fs_id.clone(),
            mode: 33188,
