...
use nanofile_server::serialization::commit_json::CommitData;
use nanofile_server::serialization::fs_json::{DirEntryData, FsDirData, FsFileData};
use nanofile_server::serialization::pack_fs;
...
async fn test_fs_dir_serialization_roundtrip() {
    ...
    let fs_id = nanofile_server::crypto::fs_id::sha1_hex(dir_data.to_compact_json().as_bytes());
...
async fn test_fs_file_serialization_roundtrip() {
    ...
    let fs_id = nanofile_server::crypto::fs_id::sha1_hex(file_data.to_compact_json().as_bytes());
    assert_eq!(fs_id.len(), 40);
...
async fn test_recv_fs_binary_format() {
    ...
    let client = server.client();
    use nanofile_server::crypto::fs_id::sha1_hex;
...
async fn test_regression_recv_fs_stores_correct_dir_type() {
    ...
    let dir_fs_id = nanofile_server::crypto::fs_id::sha1_hex(dir_data.to_compact_json().as_bytes());
...
async fn test_regression_fs_id_is_stable() {
    ...
    let fs_id_1 = nanofile_server::crypto::fs_id::sha1_hex(dir_data.to_compact_json().as_bytes());
    let fs_id_2 = nanofile_server::crypto::fs_id::sha1_hex(dir_data.to_compact_json().as_bytes());
    ...
    let fs_id_wrong =
        nanofile_server::crypto::fs_id::sha1_hex(dir_data_wrong_type.to_compact_json().as_bytes());
    assert_ne!(
        fs_id_1, fs_id_wrong,
...
async fn test_resolve_fs_id_deep_path() {
    ...
    let sub_fs_id = nanofile_server::crypto::fs_id::sha1_hex(sub_dir.to_compact_json().as_bytes());
    let sub_json = sub_dir.to_compact_json();
    let sub_compressed = pack_fs::compress_fs_data(sub_json.as_bytes()).unwrap();

    let root_dir = make_dir_fs_data(vec![DirEntryData {
        id: sub_fs_id.clone(),
        mode: nanofile_server::serialization::S_IFDIR,
        modifier: "test@example.com".to_string(),
        mtime: chrono::Utc::now().timestamp(),
        name: "sub".to_string(),
        size: 0,
...
async fn test_read_fs_dir_data_invalid_fs_id() {
    ...
    let result = nanofile_server::repo::read_fs_dir_data(
        server.db.as_ref(),
        &repo_id,
