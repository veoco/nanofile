use nanofile_server::serialization::fs_json::{FsDirData, FsFileData, SEAF_METADATA_TYPE_DIR};
...
fn test_fs_data_id_is_deterministic() {
    ...
    let id1 = nanofile_server::crypto::fs_id::sha1_hex(json1.as_bytes());
    let id2 = nanofile_server::crypto::fs_id::sha1_hex(json2.as_bytes());
...
fn test_store_fs_dir_object_empty_dir_returns_emty_sha1() {
    ...
    let json_hash = nanofile_server::crypto::fs_id::sha1_hex(json.as_bytes());
...
fn test_file_data_compute_fs_id_matches_json_hash() {
    ...
    let json = data.to_compact_json();
    let expected = nanofile_server::crypto::fs_id::sha1_hex(json.as_bytes());
    let actual = nanofile_server::crypto::fs_id::sha1_hex(data.to_compact_json().as_bytes());
    assert_eq!(
        actual, expected,
