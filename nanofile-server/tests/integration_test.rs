...
async fn test_regression_cdc_chunk_size_bounds() {
    ...
    let child_entries =
        nanofile_server::serialization::pack_fs::decode_pack_fs_entries(&child_packed).unwrap();
...
async fn test_root_entry_child_id_stays_in_sync() {
    ...
    let root_data_v2 =
        nanofile_server::repo::read_fs_dir_data(f.server.db.as_ref(), &f.repo_id, &root_fs_id_v2)
            .await
            .unwrap();
...
async fn test_empty_directory_uses_emtpysha1() {
    ...
    let api_fs_obj = nanofile_server::entity::fs_object::Entity::find()
        .filter(
            nanofile_server::entity::fs_object::Column::FsId
