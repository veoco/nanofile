...
async fn test_share_link_encrypted_repo_blocked() {
    ...
    let magic = infra::crypto::key_derivation::generate_magic(&repo_id, "testpass", 2, "").unwrap();
    let random_key =
        infra::crypto::key_derivation::generate_random_key_for_repo("testpass", 2, "").unwrap();
