...
impl RepoService {
    ...
    /// Get the user's default repo (first repo they created).
    ...
    /// Create a default repo for the user.
    pub async fn create_default_repo(
        ...
        let existing_members = repos.member.find_by_user_id(user_id).await?;
        let existing_member = existing_members.into_iter().next();

        if let Some(m) = existing_member {
            if let Some(r) = repos.repo.find_by_id(&m.repo_id).await? {
                // Build RepoInfo with "rw" permission for the default repo case
                return Ok((
                    build_repo_info_from_model(&r, email, &r.permission, user_id, true),
                    String::new(),
                ));
            }
        ...
        repos
            .member
            .create(crate::entity::repo_member::ActiveModel {
                id: sea_orm::NotSet,
                repo_id: Set(repo_id.clone()),
                user_id: Set(user_id),
                permission: Set("rw".to_string()),
