...
pub async fn list_repos(
    ...
    let repos =
        repo::RepoService::list_repos(state.db.as_ref(), &state.repos, auth.user_id, &auth.email)
            .await?;
    Ok(Json(repos))
}
...
pub async fn create_repo(
    ...
    let (repo_info, _token) = repo::RepoService::create_repo(
        state.db.as_ref(),
        &state.repos,
        auth.user_id,
        &auth.email,
...
pub async fn get_repo(
    ...
    let repo_info = repo::RepoService::get_repo(
        state.db.as_ref(),
        &state.repos,
        &repo_id,
        auth.user_id,
...
pub async fn delete_repo(
    ...
    repo::RepoService::delete_repo(state.db.as_ref(), &state.repos, &repo_id, auth.user_id).await?;
...
pub async fn download_info(
    ...
    let info =
        repo::RepoService::download_info(state.db.as_ref(), &state.repos, &repo_id, auth.user_id)
            .await?;
    Ok(Json(info))
}
...
pub async fn get_upload_link(
    ...
    let url = repo::RepoService::get_upload_link(
        state.db.as_ref(),
        &state.repos,
        &state.token_manager,
...
pub async fn get_update_link(
    ...
    let url = repo::RepoService::get_update_link(
        state.db.as_ref(),
        &state.repos,
        &state.token_manager,
        &state.config.server.site_url,
...
pub async fn repo_tokens(
    ...
    let result =
        repo::RepoService::repo_tokens(state.db.as_ref(), &state.repos, &repo_ids, auth.user_id)
            .await?;
...
pub async fn list_repos_v21(
    ...
    let response = repo::RepoService::list_repos_v21(
        state.db.as_ref(),
        &state.repos,
        auth.user_id,
        &auth.email,
...
pub async fn get_repo_v21(
    ...
    let repo_info = repo::RepoService::get_repo_v21(
        state.db.as_ref(),
        &state.repos,
        &repo_id,
        auth.user_id,
