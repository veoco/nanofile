...
每一项旁边会注明本平台固有的残余：macOS 必须为解析器线程放行 `fork`；Windows 容器可读取它启动所需的系统目录；媒体 worker 只允许执行配置好的 `ffmpeg`，并且只能读取交给它的那个临时文件。
...
`storage.ffmpeg_path` 指定媒体 profile 可以执行的辅助程序。容器中可能需要放行 Landlock 系统调用，等级才能达到“完整”。
