...
- **部分** —— 有资源限制**且**不可访问文件，但网络或进程约束缺一项，并列出缺失的那一项。文件层是这个等级的意义所在：一台禁掉了网络与进程、却让解析器能读取任意路径的本机不是“部分”，而是“无”。
- **无** —— 连“资源限制 + 文件隔离”都不满足，包括内核没有 Landlock、以及 Windows 上没能建出容器的情况。

每一项旁边会注明它有多重要、缺了它意味着什么，以及本平台固有的残余：macOS 必须为解析器线程放行 `fork`；Windows 容器可读取它启动所需的系统目录；媒体 worker 除 helper 本身外还被授予它所需的库。
...
媒体 profile 的“文件访问”一项带有自己的注记，因为 helper 是带库的程序：它可以读取 helper 旁边的库文件、系统库目录，以及在 macOS 上它链接的打包管理器目录（`/opt/homebrew`、`/usr/local`、`/opt/local`）。在 Linux 与 macOS 上没有任何机制限制 helper 能创建多少进程——约束来自父进程的超时与它杀掉的进程组，页面也会这样说明，而不是任由读者去推断。

两个设置决定其行为。`sandbox.enabled` 是总开关：关闭后上述功能一律不运行，也不会回退到服务器进程内解析。`sandbox.min_level` 是本机必须达到的等级——`full`、`partial`（默认）或 `none`；低于它时相关功能停用，页面会说明原因、缺失的是哪一项以及缺了它意味着什么。达到“部分”但未达“完整”的本机仍会运行这些功能，但会给出警告；未达到“部分”的本机只有在管理员把 `min_level` 设为 `none` 时才会运行，页面会把这标为不安全并解释后果。
