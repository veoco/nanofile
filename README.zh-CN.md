...
`storage.ffmpeg_path` 指定媒体 profile 可以执行的辅助程序。保存它会通过与上面两个设置相同的钩子重新解析这份授权，因此下一次媒体请求执行的就是刚配置的二进制。容器中可能需要放行 Landlock 系统调用，等级才能达到“完整”。在 Windows 上，容器目前读得到 helper 却启动不了它——`Program Files` 下的副本与用户目录下的安装同样被拒——因此那里的媒体缩略图处于停用状态，页面会报告 `media-unavailable`，而不是让缩略图默默消失；实测结果记录在模块文档里。
