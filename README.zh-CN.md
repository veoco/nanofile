...
大部分设置立即生效；监听地址、传输上限、缓存目录等在"重启"后生效。网页"系统管理 → 设置"可修改部分运行期设置，保存值优先于配置文件同名项。这些页面分为服务器、安全、身份验证、速率限制、存储、加密、维护、邮件、通知、高级十类，各自按分组列出条目，并可在浏览器里按名称、配置键或说明筛选；每一行都会说明取值的来源，以及修改后是否需要重启。

网页"系统管理 → 任务管理"分为两页。**任务列表**显示正在运行的作业、服务器持久记录的运行，以及可延迟作业正在等待的负载指标，因此一个拖慢服务器的作业可以和它看到的负载放在一起看。**已注册的任务**列出本服务器注册的每一个作业与常驻服务，并按各自的触发方式分组；每一行都用文字说明这个作业做什么、什么时候运行、以及带标签的计数，展开后可以看到它运行的策略——优先级、并发上限、超时、重试、是否会等待服务器空闲、能以什么粒度中断、崩溃后会留下什么。子系统未开启的作业也会在这里列出，并说明开启哪个开关能让它回来；可以手动运行的作业，行内就有触发按钮。
