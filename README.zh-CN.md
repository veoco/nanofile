...
网页"系统设置 → 任务管理"分为两页。**任务列表**显示正在运行的作业、服务器持久记录的运行，以及可延迟作业正在等待的负载指标，因此一个拖慢服务器的作业可以和它看到的负载放在一起看。**已注册的任务**列出本服务器注册的每一个作业与常驻服务，并按各自的触发方式分组；每一行都用文字说明这个作业做什么、什么时候运行、以及带标签的计数，展开后可以看到它运行的策略——优先级、并发上限、超时、重试、是否会等待服务器空闲、能以什么粒度中断、崩溃后会留下什么。子系统未开启的作业也会在这里列出，并说明开启哪个开关能让它回来；可以手动运行的作业，行内就有触发按钮。
