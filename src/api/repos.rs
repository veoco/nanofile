...
///
/// Uses the configured `site_url` so the returned URL is always under the
/// operator's control.  Avoids using the `Host` header which an attacker
/// can spoof to inject malicious URLs.
